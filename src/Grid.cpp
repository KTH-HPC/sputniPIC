...
void interpN2Cfield(FPfield ***vecFieldCx, FPfield ***vecFieldCy, FPfield ***vecFieldCz,FPfield ***vecFieldNx, FPfield ***vecFieldNy, FPfield ***vecFieldNz, struct grid* grd) {
    ...
    // SM: Here I changed from 1 to 0. I assume that the BC have been applied on the first. 
...
void divSymmTensorN2C(FPinterp ***divCX, FPinterp ***divCY, FPinterp ***divCZ, FPinterp ***pXX, FPinterp ***pXY, FPinterp ***pXZ, FPinterp ***pYY, FPinterp ***pYZ, FPinterp ***pZZ, grid* grd) {
    ...
    #pragma omp parallel for private(comp1X, comp2X, comp3X,comp1Y, comp2Y, comp3Y, comp1Z, comp2Z, comp3Z)
    for ( int i = 1; i < nxc - 1; i++)
        for ( int j = 1; j < nyc - 1; j++)
            #pragma clang loop vectorize(enable)
            for ( int k = 1; k < nzc - 1; k++) {
...
void curlC2N(FPfield ***curlXN, FPfield ***curlYN, FPfield ***curlZN, FPfield ***vecFieldXC, FPfield ***vecFieldYC, FPfield ***vecFieldZC, grid* grd) {
    ...
    #pragma omp parallel for private(compZDY, compYDZ, compXDZ, compZDX, compYDX, compXDY)
    for ( int i = 1; i < nxn - 1; i++)
        for ( int j = 1; j < nyn - 1; j++)
            #pragma clang loop vectorize(enable)
            for ( int k = 1; k < nzn - 1; k++) {
