...
void smoothInterpScalarC(FPinterp ***vectorC, grid* grd, parameters* param){
    ...
    if (param->SmoothON){
        ...
        for (int icount = 1; icount < param->SmoothTimes + 1; icount++) {
            ...
            applyBCscalarDensC(vectorC,grd,param); // set BC on ghost cells before smooth
            ...
            for (int i = 1; i < nxc - 1; i++)
                for (int j = 1; j < nyc - 1; j++)
                    for (int k = 1; k < nzc - 1; k++)
                        vectorC[i][j][k] = valueS * vectorC[i][j][k] + alpha * (vectorC[i - 1][j][k] + vectorC[i + 1][j][k] + vectorC[i][j - 1][k] + vectorC[i][j + 1][k] + vectorC[i][j][k - 1] + vectorC[i][j][k + 1]);
...
void smoothInterpScalarN(FPinterp ***vectorN, grid* grd, parameters* param){
    ...
    if (param->SmoothON){
        ...
        for (int icount = 1; icount < param->SmoothTimes + 1; icount++) {
            ...
            applyBCscalarDensN(vectorN,grd,param); // set BC on ghost cells before smooth
            ...
            for (int i = 1; i < nxn - 1; i++)
                for (int j = 1; j < nyn - 1; j++)
                    for (int k = 1; k < nzn - 1; k++)
                        vectorN[i][j][k] = valueS * vectorN[i][j][k] + alpha * (vectorN[i - 1][j][k] + vectorN[i + 1][j][k] + vectorN[i][j - 1][k] + vectorN[i][j + 1][k] + vectorN[i][j][k - 1] + vectorN[i][j][k + 1]);
