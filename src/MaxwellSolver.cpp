...
void MaxwellSource(FPfield *bkrylov, grid* grd, EMfield* field ,EMfield_aux* field_aux,interpDens_aux* id_aux, parameters* param){
    ...
    curlC2N(tempXN, tempYN, tempZN, field_aux->Bxc, field_aux->Byc, field_aux->Bzc, grd);
...
/** Maxwell Image */
void MaxwellImage(FPfield *im, FPfield *vector, EMfield *field, interpDensSpecies* ids, grid *grd, parameters *param){
    ...
    for (int i = 0; i < nxn; i++)
        for (int j = 0; j < nyn; j++)
            ...
            for (int k = 0; k < nzn; k++){
                vectX[i][j][k] = 0.0; vectY[i][j][k] = 0.0; vectZ[i][j][k] = 0.0;
                imageX[i][j][k] = 0.0; imageY[i][j][k] = 0.0; imageZ[i][j][k] = 0.0;
                Dx[i][j][k] = 0.0; Dy[i][j][k] = 0.0; Dz[i][j][k] = 0.0;
                tempX[i][j][k] = 0.0; tempY[i][j][k] = 0.0; tempZ[i][j][k] = 0.0;
            }
        ...
        for (int i = 0; i < nxn; i++)
            for (int j = 0; j < nyn; j++)
                ...
                for (int k = 0; k < nzn; k++) {
                                 ...
                                 denom = param->fourpi/ 2 * delt * param->dt / param->c * param->qom[is] * ids[is].rhon[i][j][k] / (1.0 + omcx * omcx + omcy * omcy + omcz * omcz);
                                 Dx[i][j][k] += (vectX[i][j][k] + (vectY[i][j][k] * omcz - vectZ[i][j][k] * omcy + edotb * omcx)) * denom;
                                 Dy[i][j][k] += (vectY[i][j][k] + (vectZ[i][j][k] * omcx - vectX[i][j][k] * omcz + edotb * omcy)) * denom;
                                 Dz[i][j][k] += (vectZ[i][j][k] + (vectX[i][j][k] * omcy - vectY[i][j][k] * omcx + edotb * omcz)) * denom;
                             }
...
/** calculate the electric field using second order curl-curl formulation of Maxwell equations */
void calculateE(grid* grd, EMfield_aux* field_aux, EMfield* field, interpDens_aux* id_aux, interpDensSpecies* ids, parameters* param){
    ...
    // krylov vectors
    FPfield *xkrylov = new FPfield[3 * (nxn - 2) * (nyn - 2) * (nzn - 2)];  // 3 E components
    FPfield *bkrylov = new FPfield[3 * (nxn - 2) * (nyn - 2) * (nzn - 2)];  // 3 components: source
    ...
    // call the GMRes
    GMRes(&MaxwellImage, xkrylov, 3 * (nxn - 2) * (nyn - 2) * (nzn - 2), bkrylov, 20, 200, param->GMREStol, field, ids, grd, param);
    
    // move from krylov space to physical space
    solver2phys(field_aux->Exth, field_aux->Eyth, field_aux->Ezth, xkrylov, nxn, nyn, nzn);
    
    // add E from Eth
    addscale(1 / param->th, -(1.0 - param->th) / param->th, field->Ex, field_aux->Exth, nxn, nyn, nzn);
    addscale(1 / param->th, -(1.0 - param->th) / param->th, field->Ey, field_aux->Eyth, nxn, nyn, nzn);
    addscale(1 / param->th, -(1.0 - param->th) / param->th, field->Ez, field_aux->Ezth, nxn, nyn, nzn);
...
void calculateB(grid* grd, EMfield_aux* field_aux, EMfield* field, parameters* param){
    ...
    curlN2C(tempXC, tempYC, tempZC, field_aux->Exth, field_aux->Eyth, field_aux->Ezth, grd);
...
void divergenceCleaning(grid* grd, EMfield_aux* field_aux, EMfield* field, interpDensNet* idn, parameters* param){
    ...
    if (!CG(xkrylovPoisson,(nxc-2)*(nyc-2)*(nzc-2),bkrylovPoisson,3000, param->CGtol, &PoissonImage, grd))
...
void calculateHatDensities(struct interpDens_aux* id_aux, struct interpDensNet* idn, struct interpDensSpecies* ids, struct EMfield* field, struct grid* grd, struct parameters* param){
    ...
    // Set J hat to zero: Important to set to zero because then it accumulate in PIdot
    for ( int i = 0; i < nxn; i++)
        for ( int j = 0; j < nyn; j++)
            for ( int k = 0; k < nzn; k++){
    ...
    applyBCscalarDensC(id_aux->rhoh,grd,param); // set BC on ghost cells before interp
    
    for (int is = 0; is < param->ns; is++) {
        divSymmTensorN2C(tempXC, tempYC, tempZC, ids[is].pxx, ids[is].pxy, ids[is].pxz, ids[is].pyy, ids[is].pyz, ids[is].pzz, grd);
        ...
        applyBCscalarDensC(tempXC,grd,param); // set BC on ghost cells before interp
        applyBCscalarDensC(tempYC,grd,param); // set BC on ghost cells before interp
        applyBCscalarDensC(tempZC,grd,param); // set BC on ghost cells before interp
        ...
        //sum(tempXN, Jxs, nxn, nyn, nzn, is); sum(tempYN, Jys, nxn, nyn, nzn, is); sum(tempZN, Jzs, nxn, nyn, nzn, is);
        for ( int i = 0; i < nxn; i++)
            for ( int j = 0; j < nyn; j++)
                for ( int k = 0; k < nzn; k++){
        ...
        for (int i = 0; i < nxn; i++)
            for (int j = 0; j < nyn; j++)
                for (int k = 0; k < nzn; k++) {
                    ...
                    omcy = beta * field->Byn[i][j][k];
                    omcz = beta * field->Bzn[i][j][k];
                    edotb = tempXN[i][j][k] * omcx + tempYN[i][j][k] * omcy + tempZN[i][j][k] * omcz;
                    denom = 1 / (1.0 + omcx * omcx + omcy * omcy + omcz * omcz);
                    id_aux->Jxh[i][j][k] += (tempXN[i][j][k] + (tempYN[i][j][k] * omcz - tempZN[i][j][k] * omcy + edotb * omcx)) * denom;
                    id_aux->Jyh[i][j][k] += (tempYN[i][j][k] + (tempZN[i][j][k] * omcx - tempXN[i][j][k] * omcz + edotb * omcy)) * denom;
                    id_aux->Jzh[i][j][k] += (tempZN[i][j][k] + (tempXN[i][j][k] * omcy - tempYN[i][j][k] * omcx + edotb * omcz)) * denom;
                }
    ...
    for ( int i = 0; i < nxc; i++)
        for ( int j = 0; j < nyc; j++)
            for ( int k = 0; k < nzc; k++)
                id_aux->rhoh[i][j][k] = idn->rhoc[i][j][k] - param->dt*param->th*tempXC[i][j][k];
