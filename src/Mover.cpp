...
int mover_PC(struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param){
    ...
    std::cout << "***  MOVER  ITERATIONS = " << part->NiterMover << " - Species " << part->species_ID << " ***" << std::endl;
    ...
    #pragma omp parallel for private(xptilde, yptilde, zptilde, uptilde, vptilde, wptilde, Exl, Eyl, Ezl, Bxl, Byl, Bzl, weight, xi, eta, zeta,omdtsq, denom, ut, vt, wt, udotb, ix, iy, iz)
...
/** particle mover  using predictor-corrector (formulated to be automatically vectorized  */
int mover_PC_V(struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param){
    ...
    for(int innter=0; innter < part->NiterMover; innter++){
        ...
        for (int ip=0; ip <  num_part; ip++){
            ...
            for (int ii=0; ii < 2; ii++)
                for (int jj=0; jj < 2; jj++)
                    for(int kk=0; kk < 2; kk++){
                        Exl[ip] += weight[ii][jj][kk]*field->Ex[ix- ii][iy -jj][iz- kk];
                        Eyl[ip] += weight[ii][jj][kk]*field->Ey[ix- ii][iy -jj][iz- kk];
                        Ezl[ip] += weight[ii][jj][kk]*field->Ez[ix- ii][iy -jj][iz -kk];
                        Bxl[ip] += weight[ii][jj][kk]*field->Bxn[ix- ii][iy -jj][iz -kk];
                        Byl[ip] += weight[ii][jj][kk]*field->Byn[ix- ii][iy -jj][iz -kk];
                        Bzl[ip] += weight[ii][jj][kk]*field->Bzn[ix- ii][iy -jj][iz -kk];
                    }
        ...
        for (int i=0; i <  num_part; i++){
            omdtsq = qomdt2*qomdt2*(Bxl[i]*Bxl[i]+Byl[i]*Byl[i]+Bzl[i]*Bzl[i]);
            denom = 1.0/(1.0 + omdtsq);
            // solve the position equation
            ...
            // solve the velocity equation
            uptilde[i] = (ut+qomdt2*(vt*Bzl[i] -wt*Byl[i] + qomdt2*udotb*Bxl[i]))*denom;
            vptilde[i] = (vt+qomdt2*(wt*Bxl[i] -ut*Bzl[i] + qomdt2*udotb*Byl[i]))*denom;
            wptilde[i] = (wt+qomdt2*(ut*Byl[i] -vt*Bxl[i] + qomdt2*udotb*Bzl[i]))*denom;
        ...
        for (int ii = 0; ii < 2; ii++)
            for (int jj = 0; jj < 2; jj++)
                for (int kk = 0; kk < 2; kk++){
                    ...
                    // Jx
                    ids->Jx[ix - ii][iy - jj][iz - kk] += part->u[i] * weight[ii][jj][kk] * grd->invVOL;
                    // Jy
                    ids->Jy[ix - ii][iy - jj][iz - kk] += part->v[i] * weight[ii][jj][kk] * grd->invVOL;
                    // Jz
                    ids->Jz[ix - ii][iy - jj][iz - kk] += part->w[i] * weight[ii][jj][kk]* grd->invVOL;
                    // pxx
                    ids->pxx[ix - ii][iy - jj][iz - kk] +=  part->u[i] * part->u[i] * weight[ii][jj][kk] * grd->invVOL;
                    // pxy
                    ids->pxy[ix - ii][iy - jj][iz - kk] +=  part->u[i] * part->v[i] * weight[ii][jj][kk] * grd->invVOL;
                    // pxz
                    ids->pxz[ix - ii][iy - jj][iz - kk] +=  part->u[i] * part->w[i] * weight[ii][jj][kk] * grd->invVOL;
                    // pyy
                    ids->pyy[ix - ii][iy - jj][iz - kk] +=  part->v[i] * part->v[i] * weight[ii][jj][kk] * grd->invVOL;
                    // pyz
                    ids->pyz[ix - ii][iy - jj][iz - kk] +=  part->v[i] * part->w[i] * weight[ii][jj][kk] * grd->invVOL;
                    // pzz
                    ids->pzz[ix - ii][iy - jj][iz - kk] +=  part->w[i] * part->w[i] * weight[ii][jj][kk] * grd->invVOL;
...
int mover_interp(struct particles* part, struct EMfield* field, struct interpDensSpecies* ids, struct grid* grd, struct parameters* param){
    
    // print species and subcycling
    std::cout << "***  MOVER + INTERP (FUSED) ITERATIONS = " << part->NiterMover << " - Species " << part->species_ID << " ***" << std::endl;
