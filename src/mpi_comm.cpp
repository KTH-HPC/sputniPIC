...
inline void _scatter(FPpart *send, FPpart *recv, long count,
                     MPI_Datatype type) {
  MPI_Scatter(send, count, type, recv, count, type, 0, MPI_COMM_WORLD);
}
...
void mpi_scatter_particles(particles *part_global, particles *part_local) {
  ...
  MPI_Scatter(part_global->q, part_local->nop, type, part_local->q,
              part_local->nop, type, 0, MPI_COMM_WORLD);

  MPI_Scatter(part_global->track_particle, part_local->nop, MPI_C_BOOL,
              part_local->track_particle, part_local->nop, MPI_C_BOOL, 0,
              MPI_COMM_WORLD);
}
