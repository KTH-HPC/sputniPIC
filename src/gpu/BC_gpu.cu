...
void applyBCids_gpu(cudaStream_t* stream, struct interpDensSpecies* ids_gpu,
  ...
  if (param->PERIODICX) {
    applyBCids_x_p_gpu<<<(th_x + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK,
                         THREADS_PER_BLOCK>>>(ids_gpu, grd_gpu);
  } else {
    applyBCids_x_np_gpu<<<(th_x + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK,
                          THREADS_PER_BLOCK>>>(ids_gpu, grd_gpu);
  }
  if (param->PERIODICY) {
    applyBCids_y_p_gpu<<<(th_y + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK,
                         THREADS_PER_BLOCK>>>(ids_gpu, grd_gpu);
  } else {
    applyBCids_y_np_gpu<<<(th_y + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK,
                          THREADS_PER_BLOCK>>>(ids_gpu, grd_gpu);
  }
  if (param->PERIODICZ) {
    applyBCids_z_p_gpu<<<(th_z + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK,
                         THREADS_PER_BLOCK>>>(ids_gpu, grd_gpu);
  } else {
    applyBCids_z_np_gpu<<<(th_z + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK,
                          THREADS_PER_BLOCK>>>(ids_gpu, grd_gpu);
  }
}
