...
void field_deallocate_device(struct EMfield* gpu_field, struct EMfield* gpu_field_ptr)
{
...
void copy_from_host_to_device(struct EMfield* gpu_field, struct EMfield* cpu_field, size_t count) 
{
    checkCudaErrors(cudaMemcpy(gpu_field->Ex_flat, cpu_field->Ex_flat, sizeof(FPfield) * count, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(gpu_field->Ey_flat, cpu_field->Ey_flat, sizeof(FPfield) * count, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(gpu_field->Ez_flat, cpu_field->Ez_flat, sizeof(FPfield) * count, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(gpu_field->Bxn_flat, cpu_field->Bxn_flat, sizeof(FPfield) * count, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(gpu_field->Byn_flat, cpu_field->Byn_flat, sizeof(FPfield) * count, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(gpu_field->Bzn_flat, cpu_field->Bzn_flat, sizeof(FPfield) * count, cudaMemcpyHostToDevice));
}

void field_alloc_and_copy_to_device(struct grid* grid, struct EMfield* gpu_field, 
                                    struct EMfield* cpu_field, struct EMfield** gpu_field_ptr)
{
    ...
    checkCudaErrors(cudaMemcpy(*gpu_field_ptr, gpu_field, sizeof(EMfield), cudaMemcpyHostToDevice));
}
