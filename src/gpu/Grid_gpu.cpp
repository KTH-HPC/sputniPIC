...
void grid_copy_to_device(struct grid* cpu_src, struct grid* gpu_dest){
    ...
    checkCudaErrors(cudaMemcpy(gpu_dest->XN_flat, cpu_src->XN_flat, count, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(gpu_dest->YN_flat, cpu_src->YN_flat, count, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(gpu_dest->ZN_flat, cpu_src->ZN_flat, count, cudaMemcpyHostToDevice));
...
void grid_copy_to_host(struct grid* gpu_src, struct grid* cpu_dest){
    ...
    checkCudaErrors(cudaMemcpy(cpu_dest->XN_flat, gpu_src->XN_flat, len, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(cpu_dest->YN_flat, gpu_src->YN_flat, len, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(cpu_dest->ZN_flat, gpu_src->ZN_flat, len, cudaMemcpyDeviceToHost));
                                    ...
                                    struct grid** grid_gpu_ptr)
{
    ...
    checkCudaErrors(cudaMemcpy(*grid_gpu_ptr, grid_gpu, sizeof(grid), cudaMemcpyHostToDevice));
}
