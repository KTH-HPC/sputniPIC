...
void particle_allocate_host(struct parameters* param, struct particles* part, int is)
{
...
void particles_positions_alloc_device(struct particles_positions_gpu* part_pos, struct particles_positions_gpu** part_pos_ptr, int length){
    ...
    checkCudaErrors(cudaMemcpy(*part_pos_ptr, part_pos, sizeof(particles_positions_gpu), cudaMemcpyHostToDevice));
}

void particles_positions_dealloc_device(struct particles_positions_gpu* part_pos, struct particles_positions_gpu* part_pos_ptr)
{
...
void particles_positions_copy_to_device(cudaStream_t* stream, struct particles* particles, struct particles_positions_gpu* part_pos, size_t idx_start_gpu, size_t idx_end_gpu, size_t idx_start_cpu){
    ...
    checkCudaErrors(cudaMemcpyAsync(part_pos->z + idx_start_gpu, particles->z + idx_start_cpu, num_particles*sizeof(FPpart), cudaMemcpyHostToDevice, *stream));
    checkCudaErrors(cudaMemcpyAsync(part_pos->u + idx_start_gpu, particles->u + idx_start_cpu, num_particles*sizeof(FPpart), cudaMemcpyHostToDevice, *stream));
    checkCudaErrors(cudaMemcpyAsync(part_pos->v + idx_start_gpu, particles->v + idx_start_cpu, num_particles*sizeof(FPpart), cudaMemcpyHostToDevice, *stream));
    checkCudaErrors(cudaMemcpyAsync(part_pos->w + idx_start_gpu, particles->w + idx_start_cpu, num_particles*sizeof(FPpart), cudaMemcpyHostToDevice, *stream));
    checkCudaErrors(cudaMemcpyAsync(part_pos->q + idx_start_gpu, particles->q + idx_start_cpu, num_particles*sizeof(FPinterp), cudaMemcpyHostToDevice, *stream));
...
void particles_positions_copy_to_host(cudaStream_t* stream, struct particles_positions_gpu* part_pos, struct particles* particles, size_t idx_start_gpu, size_t idx_end_gpu, size_t idx_start_cpu){
    ...
    checkCudaErrors(cudaMemcpyAsync(particles->z + idx_start_cpu, part_pos->z + idx_start_gpu, num_particles*sizeof(FPpart), cudaMemcpyDeviceToHost, *stream));
    checkCudaErrors(cudaMemcpyAsync(particles->u + idx_start_cpu, part_pos->u + idx_start_gpu, num_particles*sizeof(FPpart), cudaMemcpyDeviceToHost, *stream));
    checkCudaErrors(cudaMemcpyAsync(particles->v + idx_start_cpu, part_pos->v + idx_start_gpu, num_particles*sizeof(FPpart), cudaMemcpyDeviceToHost, *stream));
    checkCudaErrors(cudaMemcpyAsync(particles->w + idx_start_cpu, part_pos->w + idx_start_gpu, num_particles*sizeof(FPpart), cudaMemcpyDeviceToHost, *stream));
    checkCudaErrors(cudaMemcpyAsync(particles->q + idx_start_cpu, part_pos->q + idx_start_gpu, num_particles*sizeof(FPinterp), cudaMemcpyDeviceToHost, *stream));
 ...
 */
void particles_info_alloc_and_copy_to_device(struct particles_info_gpu* part_info_gpu, struct particles_info_gpu** part_info_gpu_ptr, struct particles* particles_cpu)
{
    // Copy data from the host object to the cpu object, the objects look different, hence the manual copy.
    ...
    checkCudaErrors(cudaMemcpy(*part_info_gpu_ptr, part_info_gpu, sizeof(particles_info_gpu), cudaMemcpyHostToDevice));
}

void particles_info_dealloc_device(struct particles_info_gpu* part_info_gpu_ptr){
...
__global__
void move_and_interpolate(struct particles_positions_gpu* part_pos, particles_info_gpu* part_info, struct EMfield* field, 
                        struct grid* grd, struct parameters* param, struct interpDensSpecies* ids_gpu, int num_particles){
...
__device__
void mover_PC_gpu(struct particles_positions_gpu* part_pos, particles_info_gpu* part_info,
                                struct EMfield* field, struct grid* grd, struct parameters* param) 
{
    int local_index = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    for (int i_sub=0; i_sub < part_info->n_sub_cycles; i_sub++)
    {
        ...
        for(int innter=0; innter < part_info->NiterMover; innter++)
        {
            ...
            // calculate weights
            xi[0]   = part_pos->x[local_index] - grd->XN_flat[get_idx(ix - 1, iy, iz, grd->nyn, grd->nzn)];
            eta[0]  = part_pos->y[local_index] - grd->YN_flat[get_idx(ix, iy - 1, iz, grd->nyn, grd->nzn)];
            zeta[0] = part_pos->z[local_index] - grd->ZN_flat[get_idx(ix, iy, iz - 1, grd->nyn, grd->nzn)];
            xi[1]   = grd->XN_flat[get_idx(ix, iy, iz, grd->nyn, grd->nzn)] - part_pos->x[local_index];
            eta[1]  = grd->YN_flat[get_idx(ix, iy, iz, grd->nyn, grd->nzn)] - part_pos->y[local_index];
            zeta[1] = grd->ZN_flat[get_idx(ix, iy, iz, grd->nyn, grd->nzn)] - part_pos->z[local_index];
        ...
        bc_particles(part_pos->x, part_pos->u, param->PERIODICX, grd->Lx, local_index); 
        bc_particles(part_pos->y, part_pos->v, param->PERIODICY, grd->Ly, local_index); 
        bc_particles(part_pos->z, part_pos->w, param->PERIODICZ, grd->Lz, local_index);                                               

    }  // end of subcycling
}

__device__
void bc_particles(FPpart* pos, FPpart* vel, bool periodic, double len, int index){
...
__device__
void interpP2G_gpu(struct particles_positions_gpu* part_pos,  particles_info_gpu* part_info, 
                    struct interpDensSpecies* ids_gpu, struct grid* grd)
{
    ...
    for (int ii = 0; ii < 2; ii++)
        for (int jj = 0; jj < 2; jj++)
            for (int kk = 0; kk < 2; kk++)
                weight[ii][jj][kk] = part_pos->q[local_index] * xi[ii] * eta[jj] * zeta[kk] * grd->invVOL;
    ...
    for (int ii = 0; ii < 2; ii++)
        for (int jj = 0; jj < 2; jj++)
            for (int kk = 0; kk < 2; kk++)
                atomicAdd(&ids_gpu->rhon_flat[get_idx((ix - ii), (iy - jj), (iz - kk), grd->nyn, grd->nzn)], 
                        weight[ii][jj][kk] * grd->invVOL);
    

    atomic_add_pressure(part_pos->u[local_index], weight, ids_gpu->Jx_flat, grd->invVOL, ix, iy, iz, grd->nyn, grd->nzn);
    atomic_add_pressure(part_pos->v[local_index], weight, ids_gpu->Jy_flat, grd->invVOL, ix, iy, iz, grd->nyn, grd->nzn);
    atomic_add_pressure(part_pos->w[local_index], weight, ids_gpu->Jz_flat, grd->invVOL, ix, iy, iz, grd->nyn, grd->nzn);

    atomic_add_pressure(part_pos->u[local_index]*part_pos->u[local_index], weight, ids_gpu->pxx_flat, grd->invVOL, ix, iy, iz, grd->nyn, grd->nzn);
    atomic_add_pressure(part_pos->v[local_index]*part_pos->v[local_index], weight, ids_gpu->pyy_flat, grd->invVOL, ix, iy, iz, grd->nyn, grd->nzn);
    atomic_add_pressure(part_pos->w[local_index]*part_pos->w[local_index], weight, ids_gpu->pzz_flat, grd->invVOL, ix, iy, iz, grd->nyn, grd->nzn);

    atomic_add_pressure(part_pos->u[local_index]*part_pos->v[local_index], weight, ids_gpu->pxy_flat, grd->invVOL, ix, iy, iz, grd->nyn, grd->nzn);
    atomic_add_pressure(part_pos->u[local_index]*part_pos->w[local_index], weight, ids_gpu->pxz_flat, grd->invVOL, ix, iy, iz, grd->nyn, grd->nzn);
    atomic_add_pressure(part_pos->v[local_index]*part_pos->w[local_index], weight, ids_gpu->pyz_flat, grd->invVOL, ix, iy, iz, grd->nyn, grd->nzn);

}

__device__
void atomic_add_pressure(FPpart part_dat, FPpart weight[2][2][2], FPinterp* ids_arr, FPfield invVOL, int ix, int iy, int iz, int stride_y, int stride_z){
    ...
    for (int ii = 0; ii < 2; ii++)
        ...
        for (int jj = 0; jj < 2; jj++)
            //#pragma unroll
            for (int kk = 0; kk < 2; kk++)
                atomicAdd(&ids_arr[get_idx((ix - ii), (iy - jj), (iz - kk), stride_y, stride_z)],
