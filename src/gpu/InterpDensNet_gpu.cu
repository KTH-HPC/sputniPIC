...
void interp_DNet_alloc_device(struct interpDensNet* idn_gpu, struct grid* grd){
    ...
    checkCudaErrors(cudaMalloc(&idn_gpu->rhoc_flat, sizeof(FPinterp)*grd->nxc*grd->nyc*grd->nzc));
 ...
 */
void interp_DNet_copy_to_device(struct interpDensNet* cpu_src, struct interpDensNet* gpu_dest, struct grid* grd){

    int length = sizeof(FPinterp) * grd->nxn * grd->nyn * grd->nzn;

    checkCudaErrors(cudaMemcpy(gpu_dest->rhon_flat, cpu_src->rhon_flat, length, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(gpu_dest->rhoc_flat, cpu_src->rhoc_flat, sizeof(FPinterp)*grd->nxc*grd->nyc*grd->nzc, cudaMemcpyHostToDevice));

    checkCudaErrors(cudaMemcpy(gpu_dest->Jx_flat, cpu_src->Jx_flat, length, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(gpu_dest->Jy_flat, cpu_src->Jy_flat, length, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(gpu_dest->Jz_flat, cpu_src->Jz_flat, length, cudaMemcpyHostToDevice));

    checkCudaErrors(cudaMemcpy(gpu_dest->pxx_flat, cpu_src->pxx_flat, length, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(gpu_dest->pxy_flat, cpu_src->pxy_flat, length, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(gpu_dest->pxz_flat, cpu_src->pxz_flat, length, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(gpu_dest->pyy_flat, cpu_src->pyy_flat, length, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(gpu_dest->pyz_flat, cpu_src->pyz_flat, length, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(gpu_dest->pzz_flat, cpu_src->pzz_flat, length, cudaMemcpyHostToDevice));
 ...
 */
void interp_DNet_copy_to_host(struct interpDensNet* gpu_src, struct interpDensNet* cpu_dest, struct grid* grd){

    int length = sizeof(FPinterp) * grd->nxn * grd->nyn * grd->nzn;

    checkCudaErrors(cudaMemcpy(cpu_dest->rhon_flat, gpu_src->rhon_flat, length, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(cpu_dest->rhoc_flat, gpu_src->rhoc_flat, sizeof(FPinterp)*grd->nxc*grd->nyc*grd->nzc, cudaMemcpyDeviceToHost));

    checkCudaErrors(cudaMemcpy(cpu_dest->Jx_flat, gpu_src->Jx_flat, length, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(cpu_dest->Jy_flat, gpu_src->Jy_flat, length, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(cpu_dest->Jz_flat, gpu_src->Jz_flat, length, cudaMemcpyDeviceToHost));

    checkCudaErrors(cudaMemcpy(cpu_dest->pxx_flat, gpu_src->pxx_flat, length, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(cpu_dest->pxy_flat, gpu_src->pxy_flat, length, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(cpu_dest->pxz_flat, gpu_src->pxz_flat, length, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(cpu_dest->pyy_flat, gpu_src->pyy_flat, length, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(cpu_dest->pyz_flat, gpu_src->pyz_flat, length, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(cpu_dest->pzz_flat, gpu_src->pzz_flat, length, cudaMemcpyDeviceToHost));
 ...
 */
void interp_DNet_dealloc_device(struct interpDensNet* idn, struct interpDensNet* idn_ptr){
 ...
 */
void interp_DNet_alloc_and_copy_to_device(
    ...
    checkCudaErrors(cudaMemcpy(*idn_gpu_ptr, idn_gpu, sizeof(interpDensNet), cudaMemcpyHostToDevice));
...
void sumOverSpecies_gpu(struct interpDensNet* idn_gpu_ptr, struct interpDensSpecies* ids_gpu_ptr, struct grid* grd_gpu_ptr, struct grid* grd){
    ...
    sum_over_species_gpu<<<blocks, THREADS_PER_BLOCK>>>(idn_gpu_ptr, ids_gpu_ptr, grd_gpu_ptr);

}

__global__
void sum_over_species_gpu(struct interpDensNet* idn, struct interpDensSpecies* ids, struct grid* grd)
{
    ...
    // density
    atomicAdd(&idn->rhon_flat[get_idx(x,y,z,grd->nyn,grd->nzn)], ids->rhon_flat[get_idx(x,y,z,grd->nyn,grd->nzn)]);
    
    atomicAdd(&idn->Jx_flat[get_idx(x,y,z,grd->nyn,grd->nzn)], ids->Jx_flat[get_idx(x,y,z,grd->nyn,grd->nzn)]);
    atomicAdd(&idn->Jy_flat[get_idx(x,y,z,grd->nyn,grd->nzn)], ids->Jy_flat[get_idx(x,y,z,grd->nyn,grd->nzn)]);
    atomicAdd(&idn->Jz_flat[get_idx(x,y,z,grd->nyn,grd->nzn)], ids->Jz_flat[get_idx(x,y,z,grd->nyn,grd->nzn)]);

    atomicAdd(&idn->pxx_flat[get_idx(x,y,z,grd->nyn,grd->nzn)], ids->pxx_flat[get_idx(x,y,z,grd->nyn,grd->nzn)]);
    atomicAdd(&idn->pxy_flat[get_idx(x,y,z,grd->nyn,grd->nzn)], ids->pxy_flat[get_idx(x,y,z,grd->nyn,grd->nzn)]);
    atomicAdd(&idn->pxz_flat[get_idx(x,y,z,grd->nyn,grd->nzn)], ids->pxz_flat[get_idx(x,y,z,grd->nyn,grd->nzn)]);
    atomicAdd(&idn->pyy_flat[get_idx(x,y,z,grd->nyn,grd->nzn)], ids->pyy_flat[get_idx(x,y,z,grd->nyn,grd->nzn)]);
    atomicAdd(&idn->pyz_flat[get_idx(x,y,z,grd->nyn,grd->nzn)], ids->pyz_flat[get_idx(x,y,z,grd->nyn,grd->nzn)]);
    atomicAdd(&idn->pzz_flat[get_idx(x,y,z,grd->nyn,grd->nzn)], ids->pzz_flat[get_idx(x,y,z,grd->nyn,grd->nzn)]);
