...
// Particles structure
#include "Particles.h"
//#include "Particles_aux.h" // Needed only if dointerpolation on GPU - avoid
//reduction on GPU
...
int main(int argc, char **argv) {
  ...
  if (!mpi_rank) {
    ...
    if (stat(param.RestartDirName.c_str(), &sb) == 0 && S_ISDIR(sb.st_mode)) {
      ...
      std::cerr << "Output directory " + param.RestartDirName +
                       " does not exists."
                << std::endl;
  ...
  double time0 = iStart;
  // avg, variance and last cycle exec time for mover, interpolation, field
  // solver and io, respectively
  ...
  for (int cycle = param.first_cycle_n;
    ...
    if (!mpi_rank) std::cout << "*** INTERPOLATION P2G ***" << std::endl;
...
#pragma omp parallel for
  ...
  if (!mpi_rank) {
    ...
    std::cout << "   Part. Sort / Cycle   (s) = " << average[0] << "+-"
              << sqrt(variance[0] / (param.ncycles - 1)) << std::endl;
    std::cout << "   Mover Time / Cycle   (s) = " << average[1] << "+-"
              << sqrt(variance[1] / (param.ncycles - 1)) << std::endl;
    std::cout << "   Interp. Time / Cycle (s) = " << average[2] << "+-"
              << sqrt(variance[2] / (param.ncycles - 1)) << std::endl;
