...
int main(int argc, char** argv) {
  ...
  if (!mpi_rank) {
    ...
    if (stat(param.RestartDirName.c_str(), &sb) == 0 && S_ISDIR(sb.st_mode)) {
      std::cout << "Output directory " + param.RestartDirName + " exists."
                << std::endl;
    } else {
      throw std::runtime_error("Output directory " + param.RestartDirName +
                               " does not exists.");
    }
  }
  ...
  if (!mpi_rank) {
    ...
    Adaptor::Initialize(param.CatalystScriptName.c_str(), 0, 0, 0, grd.nxn,
                        grd.nyn, grd.nzn, grd.dx, grd.dy, grd.dz);
  }
#endif
  ...
  if (!mpi_rank) {
    for (int is = 0; is < param.ns; is++) particle_deallocate(&part_global[is]);
  }
  ...
  //  batch_size /= num_devices; /* size computed based on free mem of one
  //  device */
  ...
  double time0 = iStart;
  // avg, variance and last cycle exec time for sort, mover, interpolation,
  // field solver and io, respectively
  ...
  for (int cycle = param.first_cycle_n;
...
#if defined(USE_CATALYST)
    if (!mpi_rank && param.CatalystCoProcessCycle > 0 &&
    ...
    if (!mpi_rank)
      std::cout << "Timing Cycle " << cycle << " : " << cycle_time[0] << " "
                << cycle_time[1] << " " << cycle_time[2] << " " << cycle_time[3]
                << " " << cycle_time[4] << std::endl;
  ...
  if (mpi_rank == 0) std::cout << "DEALLOCATED CPU RESOURCES" << std::endl;
  ...
  if (mpi_rank == 0) std::cout << "DEALLOCATED GPU RESOURCES" << std::endl;
  ...
  if (!mpi_rank) {
    ...
    std::cout << "   Paticle sort / Cycle (s) = " << average[0] << "+-"
              << sqrt(variance[0] / (param.ncycles - 1)) << std::endl;
    std::cout << "   Mover/Interp / Cycle (s) = " << average[1] << "+-"
              << sqrt(variance[1] / (param.ncycles - 1)) << std::endl;
    // std::cout << "   Interp. Time / Cycle (s) = " << average[1] << "+-" <<
    // sqrt(variance[1] / (param.ncycles - 1)) << std::endl;
    std::cout << "   Field Time / Cycle   (s) = " << average[3] << "+-"
              << sqrt(variance[3] / (param.ncycles - 1)) << std::endl;
    std::cout << "   IO Time / Cycle      (s) = " << average[4] << "+-"
              << sqrt(variance[4] / (param.ncycles - 1)) << std::endl;
    std::cout << "******************************************************"
              << std::endl;
  }
...
inline void update_statistics(double* mean, double* variance, double new_value,
                              long count) {
